...
#     seed: int,
#     wandb_run_id: Optional[str] = None,
#     max_tokens: Optional[int] = None,  # <--- NEW: Target total tokens (e.g. extend from 100k to 200k)
#     new_learning_rate: Optional[float] = None # <--- NEW: Optional LR override
# ):
#     """
#     Finetune or Resume a model from a checkpoint.

#     Args:
#         checkpoint_path: Path to the .pt checkpoint file
#         config_path: Path to the config.yaml or train_config.json
#         seed: Random seed (must match original run to ensure data continuity)
#         wandb_run_id: Optional WandB run ID to resume logging to the same run
#         max_tokens: The new total number of tokens to train up to. 
#                     If None, defaults to the value in config_path.
#         new_learning_rate: Optional new learning rate for finetuning.
#     """
#     checkpoint_path = pathlib.Path(checkpoint_path)
    
#     # 1. Parse tokens_trained from filename
#     # This tells us "where we are" in the timeline
#     match = re.search(r"tokens_(\d+)", checkpoint_path.name)
#     if not match:
#         raise ValueError(f"Could not parse token count from checkpoint filename: {checkpoint_path.name}")
#     tokens_trained_start = int(match.group(1))
    
#     # 2. Load Configuration
#     # This gives us "everything else" (model architecture, dataset process params, context window, etc.)
#     if str(config_path).endswith('.yaml') or str(config_path).endswith('.yml'):
#         config = TrainConfig.from_yaml(config_path)
#     else:
#         import json
...
#     # --- OVERRIDES ---
#     # If we want to train longer than the original plan
#     if max_tokens is not None:
#         print(f"[Finetune] Overriding total token target: {config.dataset.num_tokens} -> {max_tokens}")
#         config.dataset.num_tokens = max_tokens
    
#     # If we reached the old limit, and didn't provide a new one, stop.
#     if tokens_trained_start >= config.dataset.num_tokens:
#         raise ValueError(
#             f"Checkpoint is at {tokens_trained_start} tokens, but config.num_tokens is {config.dataset.num_tokens}. "
#             "Provide a higher `max_tokens` argument to continue training."
#         )
        
#     # Override Learning Rate if requested
#     if new_learning_rate is not None:
#         print(f"[Finetune] Overriding learning rate: {config.optimizer.learning_rate} -> {new_learning_rate}")
#         config.optimizer.learning_rate = new_learning_rate
#     # -----------------
...
#     )
#     print(f"[Finetune] Using device: {device}")
#     print(f"[Finetune] Resuming from {tokens_trained_start} tokens")

#     # 3. Resume WandB
#     if wandb_run_id:
#         print(f"[Finetune] Resuming WandB run: {wandb_run_id}")
#         import dotenv
#         dotenv.load_dotenv()
#         wandb.login(key=config.logging.wandb_api_key or os.environ.get("WANDB_API_KEY"))
#         wandb.init(
...
#             id=wandb_run_id,
#             resume="must", 
#             config=config.model_dump() # Updates WandB config with new max_tokens if changed
#         )
#         log = Log(config=config.logging)
#     else:
#         print("[Finetune] No wandb_run_id provided, starting new log context.")
#         log = config.init_logger()
...
#     # 5. Reconstruct Data State
#     # We use the seed + tokens_trained to fast-forward the random process
#     _set_random_seed(seed)
    
#     print("[Finetune] Creating dataloader and fast-forwarding dataset...")
#     # This uses config.dataset.num_tokens (which we potentially updated above)
#     # to create a generator that runs for the full duration (old + new).
#     train_dataloader = config.dataset.to_dataloader(
#         sequence_length=model.cfg.n_ctx, 
#         train=True
#     )
    
#     # Calculate how much "history" we need to skip
#     seq_len = config.dataset.sequence_length
#     samples_trained = tokens_trained_start // seq_len
#     emissions_to_burn = samples_trained * (seq_len + 1)
    
#     dataset = train_dataloader.dataset
#     if isinstance(dataset, ProcessDataset):
#         print(f"[Finetune] Burning {emissions_to_burn} emissions to match checkpoint state...")
#         for _ in tqdm(range(emissions_to_burn), desc="Restoring Data State"):
#             next(dataset.samples)
#     else:
#         print("[Warning] Dataset is not ProcessDataset, cannot guarantee deterministic fast-forward.")

#     # 6. Optimizer
#     optimizer = config.optimizer.from_model(model=model, device=device)
    
#     # 7. Analyzers (KL Divergence, etc.)
#     val_process = get_process_object(config.dataset.process, config.dataset.process_params)
#     minimum_cross_entropy = _compute_myopic_entropy(val_process, model.cfg.n_ctx, device)
#     ngram_analyzer, markov_analyzer = _setup_kl_analyzers(config=config, vocab_size=model.cfg.d_vocab)
    
#     if ngram_analyzer:
#          print("[Finetune] Note: N-Gram statistics are empty (not rebuilt from history). KL metrics may be approximate initially.")

#     # 8. Training Loop
#     print(f"[Finetune] Starting training loop until {config.dataset.num_tokens} tokens...")
    
#     # Logic to correctly log steps:
#     tokens_per_batch = config.dataset.batch_size * seq_len
#     start_batch_idx = tokens_trained_start // tokens_per_batch

#     for i, (input_data, target_data) in enumerate(tqdm(train_dataloader, desc="Finetune Loop")):
#         batch_idx = start_batch_idx + i
...
#         # Calculate REAL total tokens including the pre-finetune history
#         tokens_trained_so_far = _calculate_tokens_trained(
#             batch_size=config.dataset.batch_size,
#             sequence_len=model.cfg.n_ctx,
#             batch_idx=batch_idx
