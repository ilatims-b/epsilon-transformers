...
#         sequence_length=model.cfg.n_ctx, train=False
#     )
#     log=_compute_validation_metrics(
#         model=model,
#         eval_dataloader=eval_dataloader,
#         device=device,
#         log=log,
...
#         return_per_position=return_per_position,
#     )
#     # Separate out KL and ngram metrics
#     kl_metrics = {k: v for k, v in log.metrics["test"].items() if "kl" in k.lower()}
#     ngram_metrics = {k: v for k, v in log.metrics["test"].items() if "gram" in k.lower()}

#     persister.log_kl_metrics(tokens_trained, kl_metrics)
#     persister.log_ngram_metrics(tokens_trained, ngram_metrics)

#     # Log to wandb (plots like train/loss)
#     for name, val in kl_metrics.items():
#         metric_logger.log_metrics({f"test/{name}": val}, step=tokens_trained)
#     for name, val in ngram_metrics.items():
#         metric_logger.log_metrics({f"test/{name}": val}, step=tokens_trained)
