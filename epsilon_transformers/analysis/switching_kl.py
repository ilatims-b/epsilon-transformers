...
# ----------------- Configuration & Load -----------------
CHECKPOINT_DIR = "./p_0.9_5_mixed_ctx_100_l10_d_64_resume_4b_st"
...
# Retrieve raw markov processes as configured in train_config.json
Mess3 = PROCESS_REGISTRY["Mess3"]
p0 = Mess3(x=0.15, a=0.6)
p1 = Mess3(x=0.5, a=0.2)

T0, T1 = p0.transition_matrix, p1.transition_matrix

switch_times = [0, 24, 49, 74, 99]
switch_probs = [0.9, 0.9, 0.9, 0.9, 0.9]
...
def get_next_beliefs_and_probs(A, switch_prob_t):
    ...
    # Einsum dynamically computes: sum_{s0_old} A[0, s0_old, s1] * T0[e, s0_old, s0_new]
...
# Init joint state probabilities. Model starts completely on process 0's steady state 
# prior to the t=0 switch logic evaluating
...
for t in tqdm(range(100), desc="Searching Tree Sequences"):
    ...
    if t > 0:
        # P_{tf}(...|x_{0:t-1}) requires a sequence, ignore t=0
        N = len(current_branches)
        chunk_size = 4096
        tf_probs_all = []
        
        # Batching queries over large trees
        for i in range(0, N, chunk_size):
            chunk = current_branches[i:i+chunk_size]
            seqs = [b['seq'] for b in chunk]
            
            # Autoregressive positional padding (transformer expects context len up to 100)
        ...
        # Aggregate expectations depending conditionally on prior sequences (Expected KL)
        ...
        # Sequence-Independent KL Divergence
        P_marg /= total_prob
        Q_marg /= total_prob
        ...
        # Evaluate for exclusively sampling context initialization (t=0)
    ...
    # Sort the new branches by probability in descending order
    next_branches.sort(key=lambda x: x['prob'], reverse=True)
    # Only keep the top MAX_BRANCHES
    current_branches = next_branches[:MAX_BRANCHES]

# ----------------- Visualizations -----------------

x_axis = np.arange(1, 100) # Excludes t=0 since transformer predictions start occurring on 1

# Plot 1: Expected Sequence-Dependent KL Divergence
fig1 = go.Figure()
...
for st in [24, 49, 74, 99]:  # Plot switch points
...
# Plot 2: Sequence-Independent Marginals KL 
fig2 = go.Figure()
